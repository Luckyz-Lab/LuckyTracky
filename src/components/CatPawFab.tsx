...
export default function CatPawFab({ onClick, label = "Open Lucky AI assistant" }: CatPawFabProps) {
  ...
  return (
    <motion.button
      onClick={handleClick}
      className="fixed bottom-24 right-4 z-40 flex h-14 w-14 flex-col items-center justify-center rounded-control bg-primary text-primary-contrast shadow-card focus:outline-none focus:ring-4 focus:ring-primary/25 lg:bottom-5 lg:right-5 lg:h-16 lg:w-16 lg:rounded-full"
      aria-label={label}
      whileTap={{ scale: 0.82 }}
