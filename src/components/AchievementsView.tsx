...
export default function AchievementsView() {
  ...
  return <div className="space-y-5">
    <header className="page-header sm:flex sm:items-end sm:justify-between"><div className="max-w-2xl"><h1 className="page-title">Achievements</h1><p className="page-subtitle max-w-xl">Progress comes from real entries, budgets and savings goals. Rewards are shared with the active household.</p></div><div className="mt-5 flex gap-3 sm:mt-0"><div className="rounded-control bg-surface-muted px-4 py-3"><p className="text-xs text-ink-muted">Unlocked</p><p className="metric-number mt-1 text-xl text-ink-strong">{unlocked}/{items.length || ACHIEVEMENTS.length}</p></div><div className="rounded-control bg-primary-soft px-4 py-3"><p className="text-xs text-primary">Claimed XP</p><p className="metric-number mt-1 text-xl text-primary">{totalXp}</p></div></div></header>
    <div className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Achievement filters">{(["all", "unlocked", "locked"] as const).map((value) => <button key={value} role="tab" aria-selected={filter === value} onClick={() => setFilter(value)} className={`min-h-11 rounded-control px-4 text-sm font-semibold capitalize transition-colors ${filter === value ? "bg-primary text-primary-contrast shadow-card" : "border border-line bg-surface text-ink"}`}>{value}</button>)}</div>
